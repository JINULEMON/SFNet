...
def evaluate_circle(pred_masks, true_masks, pred_edges, true_edges, dataset_name):
    ...
    # pred_masks, true_masks, pred_edges, true_edges = to_inputsize(pred_masks, true_masks, pred_edges, true_edges, dataset_name)
...
if __name__ == '__main__':
    import torchvision
    #msk = cv2.imread('D:\irisseg\IrisParseNet-master\evaluation\localization\examples\iris_boundary_circle_edge\S4090L08_03315.png')
    #gt = cv2.imread('D:\irisseg\IrisParseNet-master\evaluation\localization\examples\iris_boundary_circle_edge\S4090L08_03315.png')
    msk= cv2.imread('/data/home/xc/iris_segmentation/train/iris-edgevit-Ml/experiments/M1-EdgeViT_20220915_012751_logging/checkpoints/test/SegmentationClass/M1001D01.png')
    gt = cv2.imread('/data/home/xc/iris_data/CASIA-Iris-M1/test/SegmentationClass/M1001D01.png')
    true_mask = torchvision.transforms.ToTensor()(gt)[0].unsqueeze(0)
    pred_mask = torchvision.transforms.ToTensor()(msk)[0].unsqueeze(0)
    print(true_mask.size())
    print(true_mask)
    print(pred_mask.size())
    print(pred_mask)
    hsdf = compute_hsdf(1, pred_mask, true_mask)
    print(hsdf)
