...
from datasets.canny_M1_UB_MI_Dis import wcyDataset
...
dataset_name = 'MICHE'
example_name = 'MICHE-SFNet'

#pretrain_model_path ="/data/home/xc/iris_segmentation_open/experiments/MICHE-SFNet_20221220_051350_logging/checkpoints/logging.pth"
pretrain_model_path="/data/home/xc/experiments/miche-SFNet_20240626_142353_logging/checkpoints/logging.pth"
def get_args():

    parser = argparse.ArgumentParser(description='Test paprmeters',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--log', type=str, default='logging.log', dest='log_name')
    parser.add_argument('--ckp', type=str, default=pretrain_model_path, help='Load checkpoints from a xxx/xxx.pth',
                        dest='checkpoints')
...
def main(train_args):
    ...
    test_dataset = wcyDataset(dataset_name, mode='test', transform=test_augment)
...
def test(test_loader, net):
    ...
    for vi, data in enumerate(test_loader):
        ...
        #################### val for pupil edge ##################
        pupil_val_results = evaluate_circle(pred_pupil_circle_mask, pupil_mask, pred_pupil_egde, pupil_edge,
                                            dataset_name)
        ...
        pred_iris_circle_mask_pil = transforms.ToPILImage()(
            (pred_iris_circle_mask[0] > 0).to(dtype=torch.uint8) * 255).convert('L')
        ...
        pred_pupil_circle_mask_pil = transforms.ToPILImage()(
            (pred_pupil_circle_mask[0] > 0).to(dtype=torch.uint8) * 255).convert('L')
